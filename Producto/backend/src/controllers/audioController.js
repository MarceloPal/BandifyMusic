/**
 * Controlador de Audio — pipeline de upload + análisis IA.
 *
 *   1. uploadUrl   — emite URL presignada para PUT a S3 (5 min)
 *   2. listenUrl   — emite URL presignada para GET desde S3 (1 hora, soporta Range)
 *   3. analyze     — encola job en local + IA Service, gating freemium (≤3 demos)
 *   4. obtenerJob  — polling: si IA terminó, persiste vector + metadata + s3_key MP3
 */
 ...
 * Devuelve { uploadUrl, s3Key } válida 5 minutos.
 */
...
/**
 * GET /audio/listen-url?key=demos/xxx
 * URL firmada para reproducción (1 hora). S3 soporta HTTP 206 Range Requests.
 */
exports.listenUrl = async (req, res, next) => {
  try {
    const { key } = req.query;
    ...
    const contentType = AUDIO_EXT_MIME[ext] || 'audio/mpeg';
...
/**
 * POST /audio/analyze
 * Crea job local + envía s3_key al IA Service. Aplica gating freemium (≤3 demos).
 */
 ...
 */
exports.obtenerJob = async (req, res, next) => {
  try {
